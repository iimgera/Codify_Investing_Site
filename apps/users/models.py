...
class BusinessOwner(models.Model):
    ...
    id, user, first_name, last_name, photo, sphere, business_name,
    contact_info, active
    """
    ...
    photo = models.ImageField(
        upload_to='business_owners', null=True, blank=True,
        verbose_name='Фото', help_text='Фотографии для загрузки'
    )
    sphere = models.CharField(
        max_length=100, null=True, blank=False, 
        verbose_name='Сфера', help_text='Сфера деятельности'
    )
    business_name = models.CharField(
        max_length=100, null=True, blank=False, verbose_name='Название бизнеса'
    )
    contact_info = models.TextField(
        null=True, blank=False, verbose_name='Контактная информация',
        help_text='Для связи'
    )
    active = models.BooleanField(default=True)
